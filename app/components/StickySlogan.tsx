...
export default function StickySlogan({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      // Fades out early: starts as the process section rises to ~0.78vh and is
      // completely gone by ~0.55vh — still before the "Our process" title
      // (which rides at the section top) reaches the slogan.
      const p = Math.min(1, Math.max(0, (vh * 0.78 - top) / (vh * 0.23)));
